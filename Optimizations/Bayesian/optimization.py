...
def bayesian_optimization_fit(df1, df2, predictor_columns, task_name, n_calls=50, tol=1e-4, n_iter_no_change=10,
    ...
    # Extract the 'Normalized Average' as the target variable
    y_true = df1['Normalized Average'].values

    # Ensure that df1 and df2 are aligned
    if 'image' in df1.columns and 'image' in df2.columns:
        df_combined = pd.merge(df1, df2, on='image')
        y_true = df_combined['Normalized Average'].values
        df2 = df_combined[predictor_columns]
    else:
        df2 = df2[predictor_columns]

    X = df2.values  # shape (n_samples, n_predictors)
...
def optimize_weights(X, y_true, predictor_columns, task_name, n_calls, tol, n_iter_no_change, n_repeats, opt_dir):
    ...
    for i in range(n_predictors):
        ...
        weights[i] = 1
        y_pred = X @ weights  # Linear combination of predictors
